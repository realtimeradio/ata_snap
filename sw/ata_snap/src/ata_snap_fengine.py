...
class AtaSnapFengine(object):
    ...
    def spec_test_vector_mode(self, enable):
        ...
        if enable:
            ...
            self.logger.info("Turning OFF post-EQ test-vectors")
        self.fpga.write_int('spectvg_tvg_en', int(enable))
    ...
    def select_output_channels(self, start_chan, n_chans, dests=['0.0.0.0']):
        ...
        for slot in range(n_available_slots):
            ...
            if not slot_is_valid[slot]:
                self._assign_chans([0]*self.n_chans_per_packet, slot*2*self.n_chans_per_packet)
            else:
    ...
    def _assign_chans(self, channel_range, start_index):
        ...
        self.logger.debug("Reordering channels %s to start at index %d" % (channel_range, start_index))
        channel_range = [x//4 for x in channel_range[0::4]]
        channel_range_str = struct.pack('>%dL' % (len(channel_range)), *channel_range)
        self.fpga.write("chan_reorder_reorder3_map1", channel_range_str, offset=4*start_index//4)
