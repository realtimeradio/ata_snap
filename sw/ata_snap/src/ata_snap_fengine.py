...
class AtaSnapFengine(object):
    ...
    n_chans_out = 2048 # Maximum number of channels output by SNAP per interface
    n_chans_per_block = 16 # Granularity with which we can set channels
    n_interfaces = 2 # Number of available 10GbE interfaces

    n_chans_per_packet = 256 # Number of channels per output voltage packet
    ...
    def select_output_channels(self, start_chan, n_chans, dests=['0.0.0.0'], n_interfaces=None):
        """
        ...
        :raises AssertionError: If the following conditions aren't met:
            ...
            `n_chans` should be <= n_interfaces * self.n_chans_out (2048)
        ...
        assert n_chans <= n_interfaces*self.n_chans_out
        ...
        # It's acceptable to have, eg, 4 destinations specified, where 3 are the same
        # which would give asymmetric transmission. But you can't (eg) supply
        # 3 packets of channels and two destinations.
        ...
        n_available_slots_per_if = self.n_chans_out // self.n_chans_per_packet
        n_available_slots = n_interfaces_req * n_available_slots_per_if
        ...
        for i in range(self.n_interfaces):
            for slot in range(n_available_slots_per_if):
                # IP of 0 indicates "do not send"
                self.fpga.write_int('packetizer%d_ips' % i, 0, word_offset=slot)

        # Now enable slots as required
        for slot in range(n_available_slots):
            # which 10GbE port to use?
            # alternate between required ports
            interface = slot % n_interfaces_req
            sc = slot_chan[slot]
            ...
            if not slot_is_valid[slot]:
                self._assign_chans(range(self.n_chans_per_packet), out_c)
            else:
                self._assign_chans(range(sc, sc + self.n_chans_per_packet), out_c)
