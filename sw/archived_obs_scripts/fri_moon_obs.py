    ...
    for freq in freqs:
        for antn, ant in enumerate(ants):
            for onoff_n, onoff in enumerate(["on", "off"]*NONOFFS):
                ...
                proc = Popen(["python", "take_spec_data.py", "snap1", "/home/sonata/dev/ata_snap/snap_adc5g_spec/outputs/snap_adc5g_spec_2018-06-16_1616.fpg", "-n", "%d" % NINTS, "-c", "%s_%s%d_ant_%s_%.2f" % (source, onoff, onoff_n,  ant, freq)])
                proc.wait()
